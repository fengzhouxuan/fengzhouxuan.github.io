...
```text
矩阵 × 向量：输入一个点或方向，输出一个点或方向。
矩阵 × 矩阵：输入一个变换，输出两个变换合成后的新变换。
```
...
所以 `2 × 2` 矩阵乘一个 `2 × 1` 的列向量，结果确实是 `2 × 1`，也就是一列；但两个 `2 × 2` 矩阵相乘，右边有两列，结果自然也有两列：
...
右边的 `B` 离向量 `v` 最近，因此**先做 B，再做 A**。`A × B` 的意义，就是把“先 B、后 A”压缩成一个新的矩阵。

#### 两列从哪里来：把 B 的两根轴再交给 A
...
`b₁` 是原 x 轴经过 `B` 后的位置，`b₂` 是原 y 轴经过 `B` 后的位置。再做 `A`，就是分别把这两根已经变形的轴送进 `A`：
...
> `A × B` 的第一列，是原 x 轴先经 B、再经 A 后的去向；第二列对原 y 轴也是同样的过程。

因此，矩阵乘法的结果并不是“一列计算完了就结束”，而是在逐列回答：两根输入基向量最终各去了哪里。
...
左上角这个格子，是 `A` 的第一行和 `B` 的第一列做点乘：

```text
[a, b] · [p, r] = ap + br
```

它表示：`B` 的第一根新轴 `(p, r)` 再经过 `A` 后，最终 **x 分量** 是多少。因为 `A` 的第一行本来就是计算输出 x 分量的规则：

```text
x' = a × x + b × y
```

完整结果只是把“每一根 B 的列”和“每一条 A 的输出规则”都配对一次：
...
> 列回答“输入的哪根轴最后去了哪里”；行回答“最终某个坐标分量该怎样计算”。

#### 一个既能算、又能看的例子

令 `B` 把 x 方向拉长两倍，`A` 把平面逆时针旋转 90°：
...
第一列 `(0, 2)` 和第二列 `(-1, 0)`，就是组合变换后的两根新基向量。你不需要先记住四个格子的公式；先看懂“把 B 的每一列继续交给 A”，公式就会自然出现。

{% note info %}
本节只用 `2 × 2` 讨论缩放、旋转等会把原点仍留在原点的线性变换；平移为什么需要更大的矩阵，会在下一节的齐次坐标解释。
{% endnote %}
...
<div class="interactive-demo">
  <iframe
    class="interactive-demo__frame"
    src="/rabbit-holes/rendering/matrix-composition-lab/"
    title="矩阵乘法与组合变换交互演示"
    loading="lazy"
    referrerpolicy="no-referrer"
  ></iframe>
  <div class="interactive-demo__actions">
    <a href="/rabbit-holes/rendering/matrix-composition-lab/" target="_blank" rel="noopener">在新页面打开演示</a>
  </div>
</div>

先保持默认值：`B` 做 x 方向缩放，`A` 做旋转。观察三个图中的单位正方形，以及结果矩阵的两列。再分别拖动两个滑块，确认 `A × B` 总是“先 B，后 A”。
